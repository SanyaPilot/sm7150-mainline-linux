...
static ssize_t aw3644_ir_write(struct file *filp, const char *buf, size_t len, loff_t *fseek)
{
	int ret = 0;
...
static ssize_t aw3644_ir_read(struct file *filp, char *buf, size_t len, loff_t *fseek)
{
	int ret = 0;
	int data = 0;
...
static long aw3644_ir_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	struct aw3644_chip_data *chip = filp->private_data;
	aw3644_data params;
	...
	if ((_IOC_DIR(cmd) & _IOC_WRITE) || (_IOC_DIR(cmd) & (_IOC_WRITE|_IOC_READ))) {
		if (copy_from_user(&params, (void __user *)arg, sizeof(aw3644_data))) {
...
#ifdef CONFIG_COMPAT
static long aw3644_ir_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
...
static int aw3644_dump_reg(struct device *dev)
{
	...
	int i = 0;
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct aw3644_chip_data *chip = container_of(led_cdev, struct aw3644_chip_data, cdev_ir);
				 ...
				 const char *buff, size_t count)
{
	...
	dev_err(chip->dev, "aw3644_reg_opt_store, reg:0x%x, val_after_set:0x%x.\n", addr, val);
...
static ssize_t aw3644_id_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct aw3644_chip_data *chip =
...
static DEVICE_ATTR(pwm_period, S_IRUGO | S_IWUSR,
	aw3644_pwm_period_show, aw3644_pwm_period_store);
static DEVICE_ATTR(pwm_duty, S_IRUGO | S_IWUSR,
	aw3644_pwm_duty_show, aw3644_pwm_duty_store);
static DEVICE_ATTR(id, S_IRUGO,
	aw3644_id_show, NULL);
static DEVICE_ATTR(reg_opt, S_IRUGO | S_IWUSR,
	aw3644_reg_opt_show, aw3644_reg_opt_store);

static struct attribute *aw3644_ir_attrs[] = {
...
static struct aw3644_platform_data *aw3644_parse_dt(struct i2c_client *client)
{
	...
	pdata->ito_detect_gpio = of_get_named_gpio(np, "aw3644,ito-detect-gpio", 0);

	pdata->pass_mode = of_property_read_bool(np,
			"aw3644,pass-mode");

	pdata->use_simulative_pwm = of_property_read_bool(np,
			"aw3644,use-simulative-pwm");
	...
	if (pdata->use_simulative_pwm) {
		ret = of_property_read_u32(np,
			"aw3644,period-us", &pdata->pwm_period_us);
		if (ret < 0) {
		...
		ret = of_property_read_u32(np,
			"aw3644,duty-us", &pdata->pwm_duty_us);
		if (ret < 0) {
